...
#lOADS WAV DATA FROM THE FILE LOCATION AND CONVERT IT TO FFT COMPONENTS OF FIRST 1000 FEATURES AND STORE IT AS AN ARRAY AND GENRE ALSO STORED AS AN ARRAY EACH GENRE IS GIVEN A VARIABLE
def loadFFTData():
    data=[]
    genre_list=[]
...
def main():
    ...
    #APPENDING BIAS 1 TO THE FEATURES SO INPUT IS 600*1001
    ...
    for i in range (len(fold_split_data)):
        weights_init = np.zeros(shape=(6,1001))
        train_data_indeces,test_data_indeces  = fold_split_data[i]
        train_data=np.zeros((540,1001),dtype=float)
        test_data=np.zeros((60,1001),dtype=float)
        ...
        #calling logistic regression method
        ...
        #    genre[k]=genre_list[test_data_indeces[i][k]-1]
        ...
        #confusion=confusion_matrix(genre,classify)
        print confusion
