...
def nb(beta):
    #initial declaration of class label,vocabulary list,trainig size and esting size
     ...
     trainingsize = 11269;
     testingsize = 7505;
    #beta value retrieved from the main method where beta=1/|V|
     beta1 = beta
     alpha = 1 + beta1
     #print beta
     #print alpha
    ...
    #MAP CALCULATION USING NUMPY INDICES
     ...
     #print testdatamatrix
     testdatamatrix_transpose=testdatamatrix.transpose()
...
#TOP 100 WORDS FOR RANKING WITH HIGH INFO
     exy = np.zeros(shape=(20,61188))
     xp = np.zeros(shape=(1,61188))
     ex = np.zeros(shape=(1,61188))
     ix = np.zeros(shape=(1,61188))
     for i in range(0,20):
         for j in range(0,61188):
             exy[i][j]=(-(map1[i][j]*(np.log2(map1[i][j])))-((1-map1[i][j])*(np.log2(1-map1[i][j]))))*mle[i]
             xp[0][j]=xp[0][j]+(map1[i][j]*mle[i])
     #print"exy", exy
     for k in range(0,61188):
         ex[0][k]=-(xp[0][k]*np.log2(xp[0][k]))-((1-xp[0][k])*np.log2(1-xp[0][k]))
     sumexy=np.sum(exy,axis=0)
     #print "sum exy",sumexy
     #print "ex",ex
     for l in range(0,61188):
         #ix[0][l]=sumexy[l]
         ix[0][l]=ex[0][l]-sumexy[l]
     #ix[0]=ex[0]-sumexy
     #print ix
     index1=np.argsort(ix)
     #print index1
     #vocabulary{index1(1:100)}
     testind=1
     i=61187
     #for i in range(61187,61087):
     while(i>=61087):
         print testind, vocablist1[index1[0][i]]
         i=i-1
         testind=testind+1

     return accuracy
#MAIN METHOD TO METHOD FOR CALCULATING NAIVE BAYES
def main():
    vocab = 61188;
